...
def render():
    ...
    MODEL_PATH = "models/asm_pipeline.joblib"
    ...
    with mid:
        ...
        if trained_asms:
            ...
            asm_opts = [SELECT, "Levetiracetam", "Lamotrigine", "Valproate", "Phenobarbital",
                        "Carbamazepine", "Phenytoin", "Other"]
        ...
        if submitted:
            ...
            missing = [k for k in required_keys if sample_patient.get(k) is None]
            if missing:
            ...
            # Show risk index
            st.caption(f"Clinical risk index (0–5): {risk_index}  |  Model: {artifacts.get('model_name','(unknown)')}  |  Threshold: {float(artifacts['threshold']):.2f}")

            # Reliability warnings
            if flags:
                st.warning("⚠️ Reliability warning: this input is outside typical training support.\n\n- " + "\n- ".join(flags))

            # Risk-based interpretability
            if risk_index >= 4 and prob > 0.50:
                st.warning("⚠️ High clinical risk profile detected. Probability has been capped for realism.")
